...
document.getElementsByClassName("addButton")[0].src = browser.runtime.getURL(
...
<tr class="form">
    ...
    <td><input class="deleteButton" type="image" src="${browser.runtime.getURL(
...
function setForm() {
  ...
  browser.storage.sync.get(["settings"], function(value) {
    if (value.settings.length == 0) addForm();
    //設定が無い時
    else {
